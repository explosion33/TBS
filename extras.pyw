        ...
        if done == False:
            if self.moveing[0] == False:
                ...
                if slope[1] > slope[0]:
                    ...
                    x = 1
                    y = 1
                self.moveing  = (True, pos2, time, (x,y), self.pos)

            elif self.moveing[0]:
                #print('slope', self.moveing[3])
                if self.moveing[3][0] > 0: 
                    if self.moveing[1][0] > self.moveing[4][0]:
                        g = self.moveing[1][0]
                    else:
                        g = self.moveing[4][0]
                    
                    k = g/self.moveing[3][0]
                else:
                    if self.moveing[1][1] > self.moveing[4][0]:
                        g = self.moveing[1][1]
                    else:
                        g = self.moveing[4][1]

                    k = g/self.moveing[3][1]
                #print('pre-yeet', k, self.moveing[1][0], self.moveing[3][0])
                moveTime = pygame.time.get_ticks() - self.moveTime
                k = abs(time/k * 1000)
                #print('yeet', moveTime, k)
                if moveTime >= k:
                    self.move(self.moveing[3])
                    self.moveTime = pygame.time.get_ticks()
